// ==UserScript==
// @name         Steam Economy Enhancer CH
...
/* global  luxon, jQuery, async, List, localforage */
/* eslint no-undef: off */
...
(function($, async) {
    ...
    if (currentPage == PAGE_INVENTORY) {
            ...
            getInventorySelectedMarketableItems((items) => {
                ...
                if (items.length == 0) {
                    ...
                    if (canSellSelectedItemsManually(items)) {
                        $('.sell_manual').show();
                        $('.sell_manual > span').text(`出售 ${selectedItems}${selectedItems == 1 ? ' 物品（手动） ' : ' 物品（手动） '}`);
                    } else {
                        $('.sell_manual').hide();
                    }
                    $('.sell_selected > span').text(`出售 ${selectedItems}${selectedItems == 1 ? ' 物品 ' : ' 物品 '}`);
                }
            });
        }
        ...
        function updateTurnIntoGemsButton() {
            getInventorySelectedGemsItems((items) => {
                ...
                if (items.length == 0) {
                    ...
                    $('.turn_into_gems').show();
                    $('.turn_into_gems > span').
                        text(`将 ${selectedItems}${selectedItems == 1 ? ' 物品变成宝石 ' : ' 物品变成宝石 '}`);
                }
            });
        }
            ...
            getInventorySelectedBoosterPackItems((items) => {
                ...
                if (items.length == 0) {
                    ...
                    $('.unpack_booster_packs > span').
                        text(`打开 ${selectedItems}${selectedItems == 1 ? ' 补充包 ' : ' 补充包 '}`);
                }
            });
        }
        ...
        function updateInventorySelection(selectedItem) {
            ...
            if (isBoosterPack) {
                ...
                ownerActions.append(`<br/> <a class="btn_small btn_grey_white_innerfade" href="${tradingCardsUrl}"><span>在社区市场中查看集换式卡牌</span></a>`);
            }
            ...
            if (showMiscOptions) {
                buttonsHtml += `
                    ...
                    <div class="see_inventory_buttons">
                        <a class="btn_darkblue_white_innerfade btn_medium_wide turn_into_gems" style="display:none"><span>将选中物品变为宝石</span></a>
                        <a class="btn_darkblue_white_innerfade btn_medium_wide gem_all_duplicates"><span>将所有重复物品变成宝石</span></a>
                        <a class="btn_darkblue_white_innerfade btn_medium_wide unpack_booster_packs" style="display:none"><span>打开所选补充包</span></a>
                    </div>
                `;
            } else if (TF2) {
    ...
    if (currentPage == PAGE_MARKET || currentPage == PAGE_MARKET_LISTING) {
        ...
        function processMarketListings() {
            ...
            if (currentPage == PAGE_MARKET) {
                ...
                spinnerBlock
                }<div style="text-align:center">正在加载市场列表</div>` +
                '</div>');
        ...
        function initializeMarketUI() {
            ...
            $('.my_market_header').slice(1).append(`<div class="market_listing_buttons">
                ...
                <a class="item_market_action_button item_market_action_button_green remove_selected market_listing_button">
                    <span class="item_market_action_button_contents">移除选择</span>
                </a>
            </div>`);
    ...
    function initializeTradeOfferUI() {
        ...
        $('#inventory_displaycontrols').append(`<div class="trade_offer_buttons">
            <a class="item_market_action_button item_market_action_button_green select_all">
                <span class="item_market_action_button_contents" style="text-transform:none">从页面中选择全部</span>
            </a>
        </div>`);
        ...
        const price_options = $(`<div id="see_settings_modal">
            <div>
                ...
                <select id="${SETTING_PRICE_ALGORITHM}">
                    <option value="1"${getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 1 ? 'selected="selected"' : ''}>平均历史售价最大值和最低售价列表</option>
                    <option value="2" ${getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 2 ? 'selected="selected"' : ''}>最低售价列表</option>
                    <option value="3" ${getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 3 ? 'selected="selected"' : ''}>当前最高买入订单或最低卖出清单</option>
                </select>
            </div>
    ...
    injectCss(`
        ...
        #listings_sell { text-align: center; color: #589328; font-weight:600; }
        #listings_buy { text-align: center; color: #589328; font-weight:600; }
